...
universe u_v
variable {V : Type u_v} [Fintype V] [DecidableEq V] [Nonempty V]
...
def reverse (c : G.Cycle v) : G.Cycle v where
  val := c.val.reverse
...
example (c : G.Cycle v₀) := c.val.snd

lemma snd_is_succ_start (c : G.Cycle v₀) : contains_edge c v₀ c.val.snd := by
  sorry
  -- use c.val.adj_sndOfNotNil c.prop.not_nil
  -- use c.val.firstDart_mem_darts c.prop.not_nil

theorem snd_eq_succ_start (c : G.Cycle u) (h : contains_edge c u v) : c.val.snd = v :=
...
theorem edges_eq_firstEdge_cons {G : SimpleGraph V} (c : G.Cycle v₀) :
    c.val.edges = s(v₀, c.val.snd) :: c.val.tail.edges := by
